...
object CatsIorSpec extends TestSuite {
  val tests: Tests = Tests {
    test("transform always becomes a Right") - {
    ...
    test("transform will result in a Both if a field ends up as a Both") - {
      Person("John", 10, 140)
    ...
    test("transform will result in a Left if a field ends up as a Left") - {
      Person("", 10, 140)
        .intoF[IorNec[String, +*], User]
    ...
    test("simple transforms with Ior") - {
      "success" - {
        ...
        test("3-arg (accumulate errors to the first Left") - {
          okForm
